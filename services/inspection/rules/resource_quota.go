...
func NewResourceQuotaRule(logger interfaces.Logger) interfaces.InspectionRule {
	return &ResourceQuotaRule{logger: logger}
}

func (r *ResourceQuotaRule) Name() string        { return "resource_quota" }
func (r *ResourceQuotaRule) Description() string { return "检测节点资源使用率过高的情况" }
func (r *ResourceQuotaRule) Severity() string    { return "warning" }

func (r *ResourceQuotaRule) Execute(ctx context.Context, graphDB interfaces.GraphDB) ([]inspection.InspectionResult, error) {
	query := `
	...
	for i := 0; i < resultSet.GetRowSize(); i++ {
		...
		results = append(results, inspection.InspectionResult{
			RuleName:   r.Name(),
			Severity:   r.Severity(),
			Message:    fmt.Sprintf("节点 %s 运行 %d 个 Pod，资源使用率可能过高", ns, pc),
			Resources:  []string{fmt.Sprintf("Node:%s", ns)},
			Suggestion: "检查节点资源使用率，考虑扩容或迁移部分 Pod",
			Timestamp:  time.Now(),
		})
	}
