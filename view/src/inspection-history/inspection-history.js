...
createApp({
  ...
  setup() {
      ...
      try {
        ...
        const toRFC3339 = (d) => {
          ...
          if (
            d.endsWith('Z') ||
            d.includes('+') ||
            (d.includes('T') && d.split('T')[1].length >= 8)
          )
            return d
        ...
        const params = {
          ...
          start:
            toRFC3339(filters.value.start) || now.toISOString().replace(/T.*/, 'T00:00:00.000Z'),
    ...
    const goPage = (p) => {
      if (p >= 1 && p <= totalPages()) fetchList(p)
    }
    ...
    const loadAllReportOptions = async () => {
      try {
        ...
        const res = await API.inspection.list({
          limit: 200,
          offset: 0,
  ...
  methods: {
    ...
    renderFilters() {
      return h('div', { class: 'card filters-bar' }, [
        h('input', {
          type: 'datetime-local',
          placeholder: '开始时间',
          value: this.filters.start,
          onInput: (e) => (this.filters.start = e.target.value),
          style: { minWidth: '180px' },
        }),
        h('input', {
          type: 'datetime-local',
          placeholder: '结束时间',
          value: this.filters.end,
          onInput: (e) => (this.filters.end = e.target.value),
          style: { minWidth: '180px' },
        }),
    ...
    renderCard(item) {
      ...
      if (critical > 0)
        statItems.push(
          h('span', { class: 'summary-stat' }, [
            h('span', { class: 'ss-dot', style: { background: SEVERITY_COLORS.error } }),
            'Critical ' + critical,
          ]),
        )
      if (warning > 0)
        statItems.push(
          h('span', { class: 'summary-stat' }, [
            h('span', { class: 'ss-dot', style: { background: SEVERITY_COLORS.warning } }),
            'Warning ' + warning,
          ]),
        )
      if (info > 0)
        statItems.push(
          h('span', { class: 'summary-stat' }, [
            h('span', { class: 'ss-dot', style: { background: SEVERITY_COLORS.info } }),
            'Info ' + info,
          ]),
        )
      statItems.push(
        h('span', { class: 'summary-stat' }, [
          h('span', { class: 'ss-dot', style: { background: '#d9d9d9' } }),
          'Total ' + total,
        ]),
      )
      ...
      if (item.severity_level)
        severityMetaItems.push(h('span', null, '严重级别: ' + item.severity_level))
      if (item.rules_checked)
        severityMetaItems.push(h('span', null, '检查规则: ' + item.rules_checked))
      ...
      return h('div', { class: 'card detail-panel' }, [
        h('div', { class: 'detail-header' }, [
          ...
          h('div', { class: 'detail-actions' }, [
            h(
              'button',
              { class: 'btn btn-primary btn-sm', onClick: this.compareFromDetail },
              '对比其他报告',
            ),
          ]),
        ]),
        ...
        h(
          ...
          [
            ...
            this.infoCard(
              ...
              h(
                'span',
                { style: { color: SEVERITY_COLORS.error, fontWeight: 600 } },
                String(critical),
              ),
            ),
            this.infoCard(
              ...
              h(
                'span',
                { style: { color: SEVERITY_COLORS.warning, fontWeight: 600 } },
                String(warning),
              ),
            ),
            this.infoCard(
              'Info',
              h('span', { style: { color: SEVERITY_COLORS.info, fontWeight: 600 } }, String(info)),
            ),
        ...
        d.issues?.length
          ? this.renderSection(
              '问题详情 (' + d.issues.length + ')',
              this.renderIssuesList(d.issues),
            )
          : null,
      ])
    },
    ...
    renderPassedRules(rules) {
      return h(
        'div',
        { class: 'passed-rules' },
        rules.map((r, idx) => h('span', { class: 'passed-rule-tag', key: idx }, r.ruleName || '-')),
      )
    },

    renderCompare() {
      const opts = this.allReportOptions.map((o) => h('option', { value: o.id }, o.label))
      return [
        h('div', { class: 'card', style: { marginTop: '20px' } }, [
          ...
          h('div', { class: 'compare-pick-row' }, [
            h(
              'span',
              { style: { fontSize: '13px', color: 'var(--text-secondary)', minWidth: '60px' } },
              '报告 1:',
            ),
            h(
              'select',
              { value: this.compareId1, onChange: (e) => (this.compareId1 = e.target.value) },
              [h('option', { value: '' }, '-- 请选择 --'), ...opts],
            ),
          ]),
          h('div', { class: 'compare-pick-row' }, [
            h(
            ...
            h(
              'select',
              { value: this.compareId2, onChange: (e) => (this.compareId2 = e.target.value) },
              [h('option', { value: '' }, '-- 请选择 --'), ...opts],
            ),
          ]),
          h(
      ...
      return h('div', { style: { marginTop: '20px' } }, [
        h('div', { class: 'card', style: { marginBottom: '16px' } }, [
          ...
          h(
            ...
            '趋势: ' +
              (trend === 'improving' ? '改善' : trend === 'worsening' ? '恶化' : '稳定') +
              ' | 新增: ' +
              newIssues.length +
              ' | 已解决: ' +
        ...
        newIssues.length
          ? this.renderSection(
              ...
              this.renderDiffIssues(newIssues, 'added'),
            )
          : null,
        resolvedIssues.length
          ? this.renderSection(
              '已解决问题 (' + resolvedIssues.length + ')',
              this.renderDiffIssues(resolvedIssues, 'removed'),
            )
          : null,
        persistentIssues.length
          ? this.renderSection(
              '持续问题 (' + persistentIssues.length + ')',
              this.renderDiffIssues(persistentIssues, 'unchanged'),
            )
          : null,
      ])
    },
    ...
    renderDiffIssues(issues, diffType) {
      const diffClassMap = {
        added: 'compare-diff-added',
        removed: 'compare-diff-removed',
        worsened: 'compare-diff-worsened',
        improved: 'compare-diff-improved',
        unchanged: '',
      }
      ...
      return issues.map((issue, idx) => {
        ...
        if (issue.suggestion)
          bodyItems.push(
            h('div', { class: 'issue-body-row', style: { color: '#389e0d' } }, issue.suggestion),
          )
    ...
    renderTrend() {
      return [
        h('div', { class: 'card', style: { marginTop: '20px' } }, [
          h('div', { class: 'trend-controls' }, [
            ...
            h(
              'select',
              {
                value: this.trendDays,
                onChange: (e) => {
                  this.trendDays = Number(e.target.value)
                },
              },
              [
            ...
            h(
              ...
              {
                class: 'btn btn-primary btn-sm',
                onClick: this.refreshTrend,
                disabled: this.trendLoading,
              },
              this.trendLoading ? '加载中...' : '刷新',
            ),
          ]),
        ]),
        ...
        this.trendLoading
          ? h('div', { class: 'loading' }, '加载中...')
          : this.trendData?.trend?.length
            ? this.renderTrendChart(this.trendData.trend)
            : h('div', { class: 'card empty-state' }, [
                h('div', { class: 'icon' }, '[ ]'),
                h('div', null, '暂无趋势数据'),
              ]),
          ...
          points.map((p, idx) => {
            ...
            return h('div', { class: 'trend-bar-row', key: idx }, [
              ...
              h('div', { class: 'trend-bars' }, [
                h('div', {
                  class: 'trend-bar-seg trend-bar-seg-pass',
                  style: { width: barPct + '%' },
                }),
              ]),
            ...
            h('span', null, [
              h('span', {
                style: {
                  ...
                  height: '12px',
                  borderRadius: '3px',
                  background: SEVERITY_COLORS.pass,
                  marginRight: '4px',
