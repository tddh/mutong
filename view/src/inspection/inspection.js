        ...
        return h('div', [
            ...
            h('div', { class: 'container' }, [
                h('div', { style: 'display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;' }, [
                    ...
                    h('button', {
                        class: 'btn',
                        style: 'padding:8px 16px;background:var(--primary-color);color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:14px;',
                ...
                this.loading
                    ...
                    : this.report
                        ? h('div', { class: 'card', style: 'margin-top:20px;padding:20px;border-left:4px solid var(--primary-color);' }, [
                            h('h3', null, '最近巡检报告'),
                            h('p', null, `报告 ID: ${this.report.id}`),
                            h('p', null, `生成时间: ${new Date(this.report.generatedAt).toLocaleString()}`),
                            h('div', { style: 'margin-top:12px;' }, [
                                h('span', { style: 'color: var(--error-color)' }, `Critical: ${(this.report.summary?.critical || 0)}`),
                                h('span', { style: 'color: var(--warning-color); margin-left: 16px;' }, `Warning: ${(this.report.summary?.warning || 0)}`)
                            ]),
                            this.report.issues?.length ? h('ul', { style: 'margin-top:12px;padding-left:20px;' }, issueList) : null
                        ])
