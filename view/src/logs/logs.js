...
createApp({
    ...
    setup() {
            ...
            try {
                ...
                if (level.value === 'error') {
                    ...
                    if (keyword.value.trim()) {
                        result = await API.logs.searchLogs(namespace.value, keyword.value.trim(), since.value, tail.value);
                    } else {
                        result = await API.logs.podLogs(namespace.value, podName.value.trim(), container.value.trim(), since.value, tail.value);
                    }
        ...
        return {
            logs, namespace, podName, container, level, keyword,
            since, tail, loading, error, lastUpdated, autoRefresh,
            fetchLogs, loadMore, onFilterSubmit, toggleAutoRefresh,
        };
    },
    render() {
                ...
                h('div', { style: 'display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;' }, [
                    h('div', { class: 'log-filter-group' }, [
                        ...
                        h('input', {
                            ...
                            onInput: (e) => { this.namespace = e.target.value; },
                            placeholder: 'default',
                        }),
                    ]),
                    h('div', { class: 'log-filter-group' }, [
                        ...
                        h('input', {
                            class: 'log-filter-input',
                            value: this.podName,
                            onInput: (e) => { this.podName = e.target.value; },
                            placeholder: '留空查全部',
                        }),
                    ]),
                    h('div', { class: 'log-filter-group' }, [
                        ...
                        h('input', {
                            ...
                            onInput: (e) => { this.container = e.target.value; },
                            placeholder: '可选',
                        }),
                    ]),
                    h('div', { class: 'log-filter-group' }, [
                    ...
                    h('div', { class: 'log-filter-group' }, [
                        ...
                        h('input', {
                            ...
                            type: 'number',
                            value: this.since,
                            onInput: (e) => { this.since = parseInt(e.target.value) || 15; },
                            min: '1',
                            style: 'width:80px;',
                        }),
                    ]),
        ...
        const logTable = () => {
            ...
            if (this.logs.length === 0) {
                return h('div', { class: 'card', style: 'text-align:center;padding:40px;color:var(--text-secondary);' }, '暂无日志数据');
            }
        ...
        return h('div', [
            ...
            h('div', { class: 'container' }, [
                h('div', { class: 'dashboard-header', style: 'display:flex;justify-content:space-between;align-items:center;' }, [
                    h('span', null, '📋 日志查询'),
                    h('div', { style: 'display:flex;align-items:center;gap:12px;' }, [
                        ...
                        this.lastUpdated
                            ? h('span', { style: 'font-size:12px;color:var(--text-secondary);' }, `最近更新: ${this.lastUpdated}`)
                            : null,
                    ]),
                ]),
                filterBar(),
