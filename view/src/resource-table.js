...
const COLORS = {
  ...
  ConfigMap: '#eb2f96',
  Secret: '#73d13d',
  PersistentVolumeClaim: '#fa8c16',
  Namespace: '#597ef7',
  Node: '#f759ab',
  Label: '#9254de',
  Event: '#8c8c8c',
  Endpoints: '#36cfc9',
  EndpointSlice: '#36cfc9',
  PersistentVolume: '#fa8c16',
  ServiceAccount: '#ff7a45',
  Role: '#9254de',
  ClusterRole: '#9254de',
  Certificate: '#ff85c0',
  CiliumEndpoint: '#2f54eb',
  ValidatingWebhookConfiguration: '#faad14',
  Result: '#b7eb8f',
}
...
createApp({
  ...
  render() {
    ...
    const ep = Math.min(s.totalPages, sp + maxV - 1)
    ...
    for (let i = sp; i <= ep; i++) {
      pageBtns.push(
        h(
          'button',
          {
            class: 'pagination-btn' + (i === s.currentPage ? ' active' : ''),
            onClick: () => {
              s.currentPage = i
              s.fetchResources()
            },
    ...
    const headerCells = [
      ...
      h(
        ...
        {
          style:
            'background:#fafafa;padding:12px;text-align:left;font-weight:600;color:#333;cursor:pointer;user-select:none;',
          onClick: () => s.sortBy(f),
        },
    ...
    const rows = s.pageData.map((r) => {
      const cells = [
        ...
        h(
          ...
          h(
            'span',
            { class: 'resource-kind-badge', style: { backgroundColor: s.getKindColor(r.kind) } },
            r.kind,
          ),
        ),
        ...
        h(
          'td',
          { class: 'expandable' },
          h('div', { class: 'expandable-content' }, s.renderLabels(r.labels)),
        ),
          ...
          h(
            ...
            h(
              'a',
              {
                ...
                style:
                  'display:inline-block;padding:4px 8px;background:#1890ff;color:white;border-radius:4px;font-size:12px;text-decoration:none;white-space:nowrap;',
                title: '打开终端',
              },
              '💻 Terminal',
            ),
    ...
    if (!rows.length && !s.loading) {
      rows.push(
        h('tr', [
          h(
            'td',
            { colspan: 8, style: 'text-align:center;padding:60px;color:#999;' },
            '没有找到匹配的资源',
          ),
        ]),
      )
    ...
    const nsOpts = [h('option', { value: 'all' }, '所有命名空间')].concat(
      s.namespaces.map((ns) => h('option', { value: ns }, ns)),
    )
    const kindOpts = [h('option', { value: 'all' }, '所有类型')].concat(
      s.kinds.map((k) => h('option', { value: k }, k)),
    )
      ...
      : s.error
        ...
        : h('div', [
            h('div', { style: 'overflow-x:auto;' }, [
              h(
                'table',
                { style: 'width:100%;border-collapse:collapse;font-size:14px;min-width:900px;' },
                [
                  h('thead', null, [
                    h('tr', { style: 'border-bottom:2px solid #e8e8e8;' }, headerCells),
                  ]),
                  h('tbody', null, rows),
                ],
              ),
            ...
            h(
              ...
              {
                style:
                  'display:flex;justify-content:flex-end;align-items:center;margin-top:20px;gap:8px;',
              },
              [
                h(
                  'span',
                ...
                h(
                  ...
                  {
                    style:
                      'padding:6px 12px;border:1px solid #d9d9d9;background:white;border-radius:4px;cursor:pointer;font-size:14px;',
    ...
    return h('div', null, [
      ...
      h('div', { class: 'container' }, [
        h(
          ...
          {
            class: 'header',
            style: 'padding:20px;border-bottom:1px solid #e8e8e8;background:#fafafa;',
          },
          [
            h(
              'h1',
              { style: 'font-size:24px;color:#333;margin-bottom:16px;' },
              'Kubernetes资源表格',
            ),
            h(
              'div',
              ...
              [
                h('div', { style: 'display:flex;flex-direction:column;gap:4px;' }, [
                  ...
                  h(
                    'select',
                    {
                      ...
                      onChange: (e) => {
                        s.namespaceFilter = e.target.value
                        s.debouncedApplyFilter()
                      },
                      style:
                ...
                h('div', { style: 'display:flex;gap:8px;' }, [
                  h(
                    ...
                    {
                      style:
                        'padding:8px 16px;border:none;border-radius:4px;font-size:14px;cursor:pointer;background:#1890ff;color:white;',
                      onClick: s.applyFilter,
                    },
                    '应用过滤',
                  ),
                  h(
                    ...
                    {
                      style:
                        'padding:8px 16px;border:none;border-radius:4px;font-size:14px;cursor:pointer;background:#52c41a;color:white;',
                      onClick: s.fetchResources,
                    },
                    '刷新数据',
                  ),
                ]),
