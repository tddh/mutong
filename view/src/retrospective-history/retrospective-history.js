...
createApp({
  ...
  setup() {
    ...
    const goPage = (p) => {
      if (p >= 1 && p <= totalPages.value) fetchList(p)
    }
        ...
        if (stored?.report_json) {
          try {
            ...
            console.error(
              ...
              parseErr.message,
              'raw:',
              stored.report_json.substring(0, 200),
            )
    ...
    const addLesson = () => {
      editForm.value.lessons_learned.push('')
    }
    const removeLesson = (i) => {
      editForm.value.lessons_learned.splice(i, 1)
    }
    const addActionItem = () => {
      editForm.value.action_items.push({
        description: '',
        priority: 'medium',
        owner: '',
        due_date: '',
    ...
    const removeActionItem = (i) => {
      editForm.value.action_items.splice(i, 1)
    }
  ...
  methods: {
      ...
      return [
        h('div', { class: 'card filters-bar' }, [
          h('input', {
            ...
            onInput: (e) => (this.filters.start_time = e.target.value),
          ...
          h('input', {
            ...
            onInput: (e) => (this.filters.end_time = e.target.value),
            style: { minWidth: '180px' },
          }),
          h(
            'select',
            {
              ...
              onChange: (e) => (this.filters.severity = e.target.value),
            },
            [
              h('option', { value: '' }, '全部级别'),
          ...
          h('input', {
            ...
            onInput: (e) => (this.filters.keyword = e.target.value),
            onKeydown: (e) => {
                ...
                [
                  ...
                  this.totalPages() > 1
                    ? h('div', { class: 'pagination' }, [
                        h(
                          'button',
                          { disabled: this.page <= 1, onClick: () => this.goPage(this.page - 1) },
                          '[<] 上一页',
                        ),
      ...
      return h('div', { class: 'report-card', onClick: () => this.viewDetail(item.fingerprint) }, [
        ...
        h('div', { class: 'card-body' }, [
          ...
          h(
            ...
            [
              item.resource_name
                ? h('span', null, `资源: ${item.resource_kind || '-'}/${item.resource_name}`)
                : null,
          ...
          item.root_cause_summary
            ? h('div', { class: 'card-summary' }, `根因: ${item.root_cause_summary}`)
            : null,
          h('div', { class: 'card-actions' }, [
            h(
              'button',
              {
                ...
                onClick: (e) => {
                  e.stopPropagation()
                  this.viewDetail(item.fingerprint)
                },
              },
              '查看详情',
            ),
            h(
              'button',
              {
                ...
                onClick: (e) => {
                  e.stopPropagation()
                  this.exportMd(item.fingerprint)
                },
              },
              '导出 Markdown',
            ),
      ...
      return h('div', { class: 'card detail-panel' }, [
        h('div', { class: 'detail-header' }, [
          ...
          h('div', { class: 'detail-actions' }, [
            this.editing
              ? null
              : h('button', { class: 'btn btn-primary btn-sm', onClick: this.startEdit }, '编辑'),
            this.editing
              ? h(
                  'button',
                  {
              ...
              : null,
            this.editing
              ? h('button', { class: 'btn btn-secondary btn-sm', onClick: this.cancelEdit }, '取消')
              : null,
            h(
              'button',
              { class: 'btn btn-secondary btn-sm', onClick: () => this.exportMd(this.selectedFp) },
              '导出 Markdown',
            ),
          ]),
        ]),
        ...
        pm.business_calls &&
        (pm.business_calls.upstreams?.length || pm.business_calls.downstreams?.length)
          ? this.renderSection('业务调用链', this.renderBusinessCalls(pm.business_calls))
          : null,
        pm.business_impact &&
        (pm.business_impact.direct_impacts?.length || pm.business_impact.indirect_impacts?.length)
          ? this.renderSection('业务影响分析', this.renderBusinessImpact(pm.business_impact))
          : null,
        pm.workload_context ? this.renderWorkloadContext(pm.workload_context) : null,
        pm.impact_assessment
          ? this.renderSection('影响评估详情', this.renderImpactAssessment(pm.impact_assessment))
          : null,
        pm.diagnosis_metrics?.length
          ? this.renderSection('指标快照', this.renderMetricsBar(pm.diagnosis_metrics))
          : null,
        pm.impacted_services?.length
          ? this.renderSection('受影响服务', this.renderImpactedSvc(pm.impacted_services))
          : null,
        ...
        this.editing
          ? this.renderEditableField(
              '📝 诊断结论',
              'root_cause',
              this.editForm.root_cause,
              pm.root_cause,
            )
          : this.renderTextField('📝 诊断结论', pm.root_cause, 'warning'),
        this.editing
          ? this.renderEditableField(
              '解决措施',
              'resolution',
          ...
          : this.renderTextField('解决措施', pm.resolution, 'success'),
        this.renderLessonsLearned(),
        pm.what_went_well?.length
          ? this.renderSection('✅ 做得好的', this.renderCheckList(pm.what_went_well, '#52c41a'))
          : null,
        pm.what_went_wrong?.length
          ? this.renderSection('❌ 可以改进的', this.renderCheckList(pm.what_went_wrong, '#ff4d4f'))
          : null,
        pm.contributing_factors?.length
          ? this.renderSection(
              '🔍 促成因素',
              this.renderCheckList(pm.contributing_factors, '#8c8c8c'),
            )
          : null,
    ...
    _initTopoGraph() {
      ...
      nodes.push({
        ...
        label: centerName,
        role: 'root_cause',
        criticality: (pm.business_context && pm.business_context.criticality) || 'high',
      })
    ...
    _initCausalGraph() {
      ...
      const pm = this.postmortem
      if (!pm || !pm.causal_chain || !pm.causal_chain.links || pm.causal_chain.links.length === 0)
        return
      ...
      pm.causal_chain.links.forEach((link, idx) => {
        ...
        if (!dedupNameMap[causeName])
          dedupNameMap[causeName] = {
            id: 'c' + idx,
            label: causeName,
            role: idx === 0 ? 'root_cause' : 'intermediate',
          }
        if (!dedupNameMap[effectName])
          dedupNameMap[effectName] = { id: 'e' + idx, label: effectName, role: 'effect' }
      ...
      const nodes = Object.values(dedupNameMap).map((d) => ({
        id: d.id,
        label: d.label.length > 30 ? d.label.slice(0, 30) + '...' : d.label,
        role: d.role,
      ...
      const edges = pm.causal_chain.links.map((link, idx) => {
        ...
        return {
          source: cid,
          target: eid,
          label: ((link.confidence || 0) * 100).toFixed(0) + '%',
      ...
      const graph = new G6.Graph({
        ...
        defaultNode: {
          type: 'rect',
          size: [160, 36],
          style: { radius: 6, fill: '#fafbfc', stroke: '#d9d9d9', lineWidth: 1 },
        },
        defaultEdge: {
          type: 'polyline',
          style: { stroke: '#aaa', lineWidth: 1.5, endArrow: true },
      ...
      graph.node((node) => {
        ...
        return {
          style: { fill: color + '15', stroke: color, lineWidth: 2 },
      ...
      graph.edge((edge) => {
        ...
        return {
          labelCfg: { autoRotate: true, style: { fill: color, fontSize: 10, fontWeight: 600 } },
          style: { stroke: color },
        }
      })
    ...
    renderTopoSection(pm) {
      if (
        ...
        return this.renderSection(
          ...
          h(
            'div',
            { style: { textAlign: 'center', padding: '40px', color: '#888', fontSize: '13px' } },
            '暂无业务调用链数据',
          ),
        )
      }
      ...
      const legend = h(
        ...
        [
          ...
          h(
            'span',
            {
              style: { cursor: 'pointer', fontWeight: 600, color: '#1890ff', userSelect: 'none' },
    ...
    renderSection(title, content) {
      return h('div', { class: 'section' }, [h('div', { class: 'section-title' }, title), content])
    },
                  ...
                  h(
                    ...
                    [
                      ...
                      u.team
                        ? h(
                            'span',
                            { style: { marginLeft: '8px', color: '#888', fontSize: '11px' } },
                            u.team,
                          )
                        : null,
                      u.criticality
                        ? h(
                            'span',
                            {
                              style: {
                                ...
                                borderRadius: '3px',
                                fontSize: '10px',
                                background: u.criticality === 'high' ? '#fff1f0' : '#f6ffed',
                                color: u.criticality === 'high' ? '#cf1322' : '#52c41a',
                              },
                            },
                            u.criticality,
                          )
                        ...
                        ? h(
                            'span',
                            {
                              style: {
                                ...
                                borderRadius: '3px',
                                fontSize: '10px',
                                background: d.criticality === 'high' ? '#fff1f0' : '#f6ffed',
                                color: d.criticality === 'high' ? '#cf1322' : '#52c41a',
                              },
                            },
                            d.criticality,
                          )
    ...
    renderBusinessImpact(bi) {
          ...
          this.infoCard(
            ...
            h(
              'span',
              {
                style: {
                  fontWeight: 600,
                  color: bi.risk_level === 'critical' ? '#cf1322' : '#faad14',
                },
              },
              bi.risk_level,
            ),
      ...
      if (bi.direct_impacts?.length)
        items.push(
          h('div', { style: { gridColumn: '1 / -1' } }, [
            h(
              'div',
              {
                style: { fontWeight: 600, fontSize: '12px', color: '#cf1322', marginBottom: '8px' },
              },
              `直接影响 (${bi.direct_impacts.length})`,
            ),
            ...bi.direct_impacts.map((d, i) =>
              h(
                'div',
                {
                  key: i,
      ...
      if (bi.indirect_impacts?.length)
        items.push(
          h('div', { style: { gridColumn: '1 / -1' } }, [
            h(
              'div',
              {
                style: { fontWeight: 600, fontSize: '12px', color: '#faad14', marginBottom: '8px' },
              },
              `间接影响 (${bi.indirect_impacts.length})`,
            ),
            ...bi.indirect_impacts.map((d, i) =>
              h(
                'div',
                {
                  key: i,
                ...
                [
                  ...
                  d.reasoning
                    ? h(
                        'div',
                        { style: { marginTop: '2px', fontSize: '11px', color: '#888' } },
                        d.reasoning,
                      )
                    : null,
                ],
      ...
      if (wc.pods?.length) {
        items.push(
          h('div', { style: { gridColumn: '1 / -1' } }, [
            h(
              'div',
              { style: { fontWeight: 600, fontSize: '12px', color: '#888', marginBottom: '8px' } },
              'Pod 列表',
            ),
            h(
              'div',
                ...
                h(
                  'span',
                  {
                    ...
                    style: {
                      ...
                      color: p.is_alerted ? '#cf1322' : '#52c41a',
                      border: '1px solid ' + (p.is_alerted ? '#ffa39e' : '#b7eb8f'),
                    },
                  },
                  p.name,
                ),
    ...
    renderImpactAssessment(ia) {
      return h('div', null, [
        ...
        ia.user_facing_impact
          ...
          : null,
        ia.affected_services?.length
          ? h('div', { style: { marginBottom: '12px' } }, [
              h(
                'div',
                {
                  style: { fontWeight: 600, fontSize: '12px', color: '#888', marginBottom: '6px' },
                },
          ...
          ? h('div', { style: { marginBottom: '12px' } }, [
              ...
              h('table', { class: 'rh-table' }, [
                ...
                h(
                  'tbody',
                  null,
                  ia.direct_impact.map((r, i) =>
                    h('tr', { key: i }, [
                      h('td', null, r.kind || '-'),
                      h('td', { style: { fontFamily: 'monospace' } }, r.name || '-'),
        ...
        ia.indirect_impact?.length
          ? h('div', null, [
              h(
                ...
                `🟡 间接影响 (${ia.indirect_impact.length})`,
              ),
              h('table', { class: 'rh-table' }, [
                h(
                  'thead',
                ...
                ia.indirect_impact.length > 20
                  ? h(
                      'tr',
                      null,
                      h(
                        'td',
                        { colSpan: 3, style: { textAlign: 'center', color: '#888' } },
                        `... 还有 ${ia.indirect_impact.length - 20} 条`,
                      ),
                    )
                  : null,
              ]),
    ...
    renderMetricsBar(metrics) {
      ...
      return h(
        ...
        metrics.map((m, idx) => {
          ...
          const color =
            m.status === 'critical' ? '#cf1322' : m.status === 'warning' ? '#faad14' : '#52c41a'
          return h(
            'div',
            { key: idx, style: { display: 'flex', alignItems: 'center', gap: '10px' } },
            [
              h(
                'span',
                {
          ...
          return h('div', { class: 'causal-link', key: idx }, [
            ...
            evidenceList.length
              ? h(
                  ...
                  evidenceList.map((evidence, evidenceIdx) =>
                    h('div', { key: evidenceIdx }, evidence),
                  ),
                )
              : null,
          ])
    ...
    renderEditableField(title, key, value, original) {
      const origText =
        typeof original === 'string' ? original : original?.final || original?.ai_generated || ''
      return h('div', { class: 'section edit-field' }, [
        ...
        origText
          ? h(
              'div',
              { class: 'ai-original' },
              `AI 原文: ${origText.slice(0, 200)}${origText.length > 200 ? '...' : ''}`,
            )
          : null,
        h('textarea', {
          value: value,
          onInput: (e) => {
            this.editForm[key] = e.target.value
          },
        }),
      ])
    },
    ...
    renderLessonsLearned() {
      const list = this.editing
        ? this.editForm.lessons_learned
        : this.postmortem?.lessons_learned || []
            ...
            h(
              'div',
              {
                ...
                style: {
                  ...
                  gap: '8px',
                  padding: '8px 0',
                  borderBottom: idx < list.length - 1 ? '1px solid #f0f0f0' : 'none',
                  alignItems: 'flex-start',
                },
              },
              [
                h(
                  'span',
                  { style: { fontWeight: 600, color: '#1890ff', flexShrink: 0 } },
                  idx + 1 + '.',
                ),
                this.editing
                  ? h('input', {
                      value: item,
                      onInput: (e) => {
                        this.editForm.lessons_learned[idx] = e.target.value
                      },
                      style: {
                        flex: 1,
                        padding: '6px 10px',
      ...
      return h('div', { class: 'section' }, [
        ...
        h('table', { class: 'rh-table' }, [
          h(
            ...
            h('tr', null, [
              h('th', null, '描述'),
              h('th', { style: { width: '80px' } }, '优先级'),
              h('th', { style: { width: '100px' } }, '负责人'),
              h('th', { style: { width: '100px' } }, '截止日期'),
                ...
                h(
                  ...
                  this.editing
                    ? h('input', {
                        ...
                        onInput: (e) => {
                          this.editForm.action_items[idx].description = e.target.value
                        },
                        placeholder: '描述',
                      })
                    : item.description || '-',
                  ...
                  this.editing
                    ? h(
                        'select',
                        {
                          ...
                          onChange: (e) => {
                            this.editForm.action_items[idx].priority = e.target.value
                          },
                        },
                        [
                ...
                h(
                  ...
                  this.editing
                    ? h('input', {
                        value: item.owner || '',
                        onInput: (e) => {
                          this.editForm.action_items[idx].owner = e.target.value
                        },
                        placeholder: '负责人',
                      })
                    : item.owner || '-',
                ...
                h(
                  ...
                  this.editing
                    ? h('input', {
                        value: item.due_date || '',
                        onInput: (e) => {
                          this.editForm.action_items[idx].due_date = e.target.value
                        },
                ...
                this.editing
                  ? h(
                      ...
                      h(
                        ...
                        {
                          class: 'btn btn-danger btn-sm',
                          onClick: () => this.removeActionItem(idx),
                        },
                        'X',
                      ),
                    )
          ...
          [
            ...
            ad.evidence?.length
              ? h('div', { style: { marginTop: '8px' } }, [
                  ...
                  ...ad.evidence.map((ev, i) =>
                    h(
                      'div',
                      { key: i, style: { padding: '4px 0', fontSize: '13px', color: '#555' } },
                      '- ' + ev,
                    ),
                  ),
                ])
