...
const COLORS = {
  ...
  ConfigMap: '#52c41a',
  Secret: '#eb2f96',
  PersistentVolumeClaim: '#13c2c2',
  PersistentVolume: '#08979c',
  Ingress: '#fa541c',
  Endpoints: '#faad14',
  EndpointSlice: '#ffc53d',
  Namespace: '#f5222d',
  StorageClass: '#2f54eb',
  ServiceAccount: '#a0d911',
  Job: '#fadb14',
  CronJob: '#d3f261',
  DaemonSet: '#6c5ce7',
  ClusterRole: '#30336b',
  ClusterRoleBinding: '#5352ed',
  Role: '#2e86de',
  RoleBinding: '#1dd1a1',
  Lease: '#c44569',
  Event: '#f39c12',
  ControllerRevision: '#576574',
  CSINode: '#3742fa',
  CSIDriver: '#487eb0',
  Certificate: '#ff6348',
  CertificateRequest: '#ff7979',
  ClusterIssuer: '#8c7ae6',
  Issuer: '#a29bfe',
  Order: '#dfe6e9',
  BusinessApp: '#e74c3c',
}
...
createApp({
  ...
  setup() {
    ...
    const businessUnits = computed(() =>
      [...new Set(allNodes.value.filter((n) => n.businessUnit).map((n) => n.businessUnit))].sort(),
    )
    ...
    const layoutModes = [{ key: 'force', label: '力导向', icon: '⚡' }]
    ...
    function extractBusinessContext(labels) {
      ...
      // 业务单元
      ctx.businessUnit =
        labels['business-unit'] ||
        labels['mutong/business-unit'] ||
        labels['app.kubernetes.io/part-of'] ||
        labels['biz-domain'] ||
        ''
      // 团队
      ctx.team =
        labels['team'] || labels['owner'] || labels['mutong/team'] || labels['managed-by'] || ''
      // 关键等级
      ctx.criticality =
        labels['criticality'] ||
        labels['mutong/criticality'] ||
        labels['priority'] ||
      ...
      ctx.environment =
        labels['environment'] ||
        labels['env'] ||
        labels['mutong/environment'] ||
        labels['stage'] ||
        ''
      // 来源
      ctx.source =
        labels['source'] ||
        labels['mutong/source'] ||
        labels['provisioner'] ||
    ...
    function parseResourceDefine(resource) {
      ...
      try {
        ...
        if (resource.kind === 'Pod') {
          ...
          if (Array.isArray(status.containerStatuses) && status.containerStatuses.length > 0) {
            result.restartCount = status.containerStatuses.reduce(
        ...
        if (resource.kind === 'CiliumEndpoint') {
          ...
          if (
            status.networking &&
            Array.isArray(status.networking.addressing) &&
        ...
        if (!result.status) {
          ...
          else if (typeof status.currentStatus === 'string' && status.currentStatus)
        ...
        if (Array.isArray(status.conditions) && status.conditions.length > 0) {
          ...
          if (!result.status) {
            const healthy = status.conditions.find(
              (c) =>
                ['Ready', 'Available', 'Succeeded', 'Completed'].includes(c.type) &&
                c.status === 'True',
            )
      ...
      if (result.status) {
        ...
        else if (s.includes('notready') || ['failed', 'error', 'lost'].includes(s))
          result.statusColor = '#ff4d4f'
    ...
    function buildForceGraph(nodes, edges) {
      ...
      if (!container) return
      if (fgInstance) {
        fgInstance._destructor && fgInstance._destructor()
      }
      ...
      fgInstance = ForceGraph()(container)
        ...
        .onNodeHover((node) => {
          ...
          if (prev && !prev.__highlighted_click) {
            prev.__highlighted = false
            prev.__highlighted_hover = false
          }
          for (let i = 0; i < fgLinks.length; i++) {
            if (fgLinks[i].__highlighted_hover && !fgLinks[i].__highlighted_click) {
              fgLinks[i].__highlighted = false
              fgLinks[i].__highlighted_hover = false
            }
          }
          for (let i = 0; i < fgNodes.length; i++) {
            if (fgNodes[i].__highlighted_hover && !fgNodes[i].__highlighted_click) {
              fgNodes[i].__highlighted = false
              fgNodes[i].__highlighted_hover = false
            }
          }
          hoveredNode = node
          if (node) {
            ...
            if (nbrs) {
              for (let i = 0; i < nbrs.l.length; i++) {
                nbrs.l[i].__highlighted = true
                nbrs.l[i].__highlighted_hover = true
              }
              for (let i = 0; i < nbrs.n.length; i++) {
                nbrs.n[i].__highlighted = true
                nbrs.n[i].__highlighted_hover = true
              }
            }
          }
        })
        .onLinkHover((link) => {
          if (hoveredLink) {
            ...
            if (srcN && !srcN.__highlighted_click) {
              srcN.__highlighted = false
              srcN.__highlighted_hover = false
            }
            if (tgtN && !tgtN.__highlighted_click) {
              tgtN.__highlighted = false
              tgtN.__highlighted_hover = false
            }
          }
          hoveredLink = link
          if (link) {
            ...
            if (srcN) {
              srcN.__highlighted = true
              srcN.__highlighted_hover = true
            }
            if (tgtN) {
              tgtN.__highlighted = true
              tgtN.__highlighted_hover = true
            }
          }
        })
        .onNodeClick((node) => {
          for (let i = 0; i < fgNodes.length; i++) {
            ...
            n.__highlighted_click = false
            n.__highlighted_hover = false
          }
          for (let i = 0; i < fgLinks.length; i++) {
            fgLinks[i].__highlighted = false
            fgLinks[i].__highlighted_hover = false
          ...
          if (nbrs) {
            ...
            for (let i = 0; i < nbrs.n.length; i++) {
              nbrs.n[i].__highlighted = true
              nbrs.n[i].__highlighted_click = true
            }
          }
          ...
          setTimeout(() => {
            if (viewMode.value === 'business') {
              ...
              fetch(
                `/k8s/resources/graph/resource-define?resourceId=${encodeURIComponent(node.id)}`,
              )
                .then((res) => (res.ok ? res.json() : null))
                .then((data) => {
                  if (
                    selectedNode.value &&
                    selectedNode.value.id === clickedNodeId &&
                    data &&
                    data.resource_define
                  ) {
                    const parsed = parseResourceDefine({
                      ...node,
                      resource_define: data.resource_define,
                    })
                    selectedNode.value = {
                      id: node.id,
                      label: node.label,
      ...
      fgNodes.forEach((n) => {
        n.__driftSeed = n.__driftSeed || Math.random() * 6283
      })
      nodeIdMap = {}
      nodeLinkMap = {}
      for (let i = 0; i < fgNodes.length; i++) {
      ...
      for (let i = 0; i < fgLinks.length; i++) {
        ...
        if (sn) {
          sn.l.push(l)
          if (nodeIdMap[tid]) sn.n.push(nodeIdMap[tid])
        }
        if (tn) {
          tn.l.push(l)
          if (nodeIdMap[sid]) tn.n.push(nodeIdMap[sid])
        }
      }

      try {
        applyAnimMode(animMode.value)
      } catch (e) {
        console.warn('[topology] applyAnimMode failed:', e)
      }
    ...
    function applyAnimMode(mode) {
      ...
      fgInstance
        .graphData()
        .nodes.forEach((n) => (n.__driftSeed = n.__driftSeed || Math.random() * 6283))
      if (mode === 'float') {
        fgInstance.cooldownTicks(Infinity).d3AlphaDecay(0.005).d3VelocityDecay(0.4)
        fgInstance.graphData().nodes.forEach((n) => {
          n.fx = undefined
          n.fy = undefined
        })
    ...
    const fetchTopology = async (uid, layers) => {
      ...
      try {
        const res = await fetch(
          `/k8s/resources/graph/search?resourceId=${encodeURIComponent(uid)}&depth=${layers}`,
        )
        ...
        const topoNodes = (data.nodes || [])
          ...
          .map((n) => ({
            ...n,
            kind: n.kind || 'Unknown',
            namespace: n.namespace || n.name_space || '-',
            _isSeed: n.id === uid,
          }))
    ...
    watch(businessUnitFilter, () => {
      if (viewMode.value === 'business') {
        ...
        const filteredNodes = allNodes.value.filter(
          (n) => n.businessUnit === unit || n.labels?.includes(unit),
        )
        ...
        const filteredEdges = allEdges.value.filter(
    ...
    const handleZoomIn = () => {
      fgInstance && fgInstance.zoom(1.5, 300)
    }
    const handleZoomOut = () => {
      fgInstance && fgInstance.zoom(0.67, 300)
    }
    const handleFitView = () => {
      fgInstance && fgInstance.zoomToFit(400, 40)
    }
