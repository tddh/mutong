...
export const API = {
    stats: { overview: () => request('/api/v1/stats/overview').catch(() => null), resourceTotal: () => request('/api/v1/stats/resource-total').catch(() => null) },
    resources: {
    ...
    metrics: {
        pod: (namespace, name) => request(`/api/v1/metrics/pod?namespace=${encodeURIComponent(namespace)}&name=${encodeURIComponent(name)}`).catch(() => null),
        node: (name) => request(`/api/v1/metrics/node?name=${encodeURIComponent(name)}`).catch(() => null),
        deployment: (namespace, name) => request(`/api/v1/metrics/deployment?namespace=${encodeURIComponent(namespace)}&name=${encodeURIComponent(name)}`).catch(() => null),
    },
    diagnosis: {
        status: () => request('/api/v1/diagnosis/status').catch(() => null),
        run: (fingerprint) => request('/api/v1/diagnosis/run', { method: 'POST', body: JSON.stringify({ fingerprint }) }),
        rerun: (fingerprint) => request('/api/v1/diagnosis/rerun', { method: 'POST', body: JSON.stringify({ fingerprint }) }),
        chat: {
            context: (data) => request('/api/v1/diagnosis/chat/context', { method: 'POST', body: JSON.stringify(data) }),
            ask: (data) => request('/api/v1/diagnosis/chat/ask', { method: 'POST', body: JSON.stringify(data) }),
        },
    },
    system: {
    ...
    clusters: {
        ...
        health: (name) => request(`/api/v1/clusters/${encodeURIComponent(name)}/health`).catch(() => null),
        stats: (name) => request(`/api/v1/clusters/${encodeURIComponent(name)}/stats`).catch(() => null),
    },
    executor: {
    ...
    inspection: {
        ...
        list: (params) => { const q = new URLSearchParams(params).toString(); return request(`/api/v1/inspection/reports${q ? '?' + q : ''}`); },
        detail: (id) => request(`/api/v1/inspection/reports/${encodeURIComponent(id)}`),
        compare: (id, otherId) => request(`/api/v1/inspection/reports/${encodeURIComponent(id)}/compare/${encodeURIComponent(otherId)}`),
    ...
    trace: {
        ...
        spans: (traceId) => request(`/api/v1/trace/spans?traceId=${encodeURIComponent(traceId)}`).catch(() => []),
    },
    retrospective: {
